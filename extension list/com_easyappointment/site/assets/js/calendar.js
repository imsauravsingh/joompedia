...
var datepickr = (function() {
		...
		calendarClick: function(e) {
			if(e.target.className) {
				switch(e.target.className) {
					...
					break;
					case 'day':
						this.container.value = formatDate(new Date(this.currentYearView, this.currentMonthView, e.target.innerHTML).getTime(), this.config);
						this.close();
					break;
				}
			}
	...
	function formatDate(milliseconds, config) {
		...
		format = {
			...
			F: function() {
				return monthToStr(dateObj.getMonth(), true, config.months);
			},
			m: function() {
				return (format.n() < 10) ? '0' + format.n() : format.n();
			},
			M: function() {
				return monthToStr(dateObj.getMonth(), false, config.months);
			},
			n: function() {
	...
	function buildNode(nodeName, attributes, content, currentMonthView, currentYearView ) {
		var element;
		...
		if ( nodeName == 'td' ) { 
			if ( content != '&nbsp;' ) {
				element.setAttribute('onclick', 'tbb.changedate(\'' + currentYearView + '-'+ currentMonthView + '-' + parseInt(content)+'\');');
			}
		}
	...
	function buildMonths(config, currentMonthView, currentYearView) {
		...
		//prevMonth = buildNode('span', { className: 'prev-month pull-left' }, buildNode('i', { className: 'prevMonth' }, '&lt;')),
		//nextMonth = buildNode('span', { className: 'next-month pull-right' }, buildNode('i', { className: 'nextMonth' }, '&gt;'));
		prevMonth = buildNode('span', { className: 'prev-month' }, buildNode('span', { className: 'ico ico-go-left' })),
		nextMonth = buildNode('span', { className: 'next-month' }, buildNode('span', { className: 'ico ico-go-right' }));
	...
	function buildDays(firstOfMonth, numDays, currentMonthView, currentYearView) {
		...
		for(i = 1; i <= numDays; i++) {
			...
			row.appendChild(buildNode('td', { className: 'day' }, i, currentMonthView, currentYearView ));
	...
	function buildCalendar() {
		...
		calendarContainer.id = 'mycal';
		calendarContainer.style.cssText = 'display: none; position: absolute; top: ' + obj.offsetTop + 'px; left: ' + obj.offsetLeft + 'px; z-index: 1100;';
	
		obj.parentNode.appendChild(calendarContainer);
		...
		var calendar = buildNode('table',  { className: 'table' } , buildNode('thead', null, buildNode('tr', { className: 'weekdays' }, buildWeekdays(this.config.weekdays))));
	...
	/*
		...
		d.m.Y - 03.12.2014
		d M Y - 03 Dec 2014
		F d, Y - December 03, 2014
		j.n.Y - 3.12.2014
		d-m-y - 03-12-14
	*/
	return function(elementId, containerId, userConfig) {
		var self = this;
		...
		this.container.value = this.config.currentTime === 0 ? '' : formatDate( this.config.currentTime, this.config );
		document.getElementById('timestamp').value = this.config.currentTime;
	}
})();
