...
class UsersModelRegistration extends JModelForm
{
	...
	public function register($temp)
	{
		...
		// added group id functionality here using form hidden value type (saurav 3-09-2016 12:09am)
		$reg_data = JRequest::get('post');
		if(isset($reg_data['type']) && !in_array($reg_data['type'],$data['groups'])){
			$data['groups'][]=$reg_data['type'];
		}
		// end
	...
	if(isset($reg_data['type']) &&  $reg_data['type']==14){
	/*  updated make_appointment staff table (saurav 03-09-2016 02:54am) */

		$db = $this->getDbo();
		$query = $db->getQuery(true);
		// Insert values.
		$values = array($user->id, $user->id, $db->quote($user->name), '1');

		$query->clear()
			->insert($db->quoteName('#__make_appointment_staff'))
			->columns($db->quoteName(array('id','user', 'name', 'published')))
