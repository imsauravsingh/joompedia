...
class EasyappointmentModelBrowse extends JModelList
{
	...
	{
    $search_type = JRequest::getVar('search_type','0','1','int');
		if ($this->getCategory()->hasChildrens() && $search_type==0)
		{
    ...
    if($search_type==1){
      $query->where($this->_db->quoteName('services') . ' !=""');
    }else{
      $query->where($this->_db->quoteName('services') . ' REGEXP ' . (int) $this->category);
		...
		foreach ($rezult as $row)
		{
			...
			if (in_array($this->category, $services) || $search_type==1)
			{
